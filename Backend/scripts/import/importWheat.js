...
/**
 * WHEAT PRODUCTION DATA IMPORTER
 * Purpose: Import 234+ wheat production records from CSV
 */

const importWheat = async () => {
  try {
    console.log("🌾 Starting Wheat Production Data Import...");
    console.log("");

    const csvPath = path.join(
      __dirname,
      "../../data/Pakistan_Wheat_Production_Data_2018-2025.csv"
    );

    // Step 1: Parse and validate CSV
    console.log("Step 1: Parsing CSV file...");
    const importer = new CSVImporter(csvPath, "WHEAT");
    ...
    validData.forEach((row, index) => {
      try {
        ...
        // Get crop type ID
        const cropTypeId = cropMap["WHEAT"];

        if (!cropTypeId) {
          console.warn(
            `  Warning: WHEAT crop type not found, skipping row ${index + 1}`
          );
          transformErrors++;
          return;
        }
        ...
        const record = {
          ...
          isForecast: false, // Wheat data is all historical/current
          reliability:
            dataSourceEnum === "PBS" || dataSourceEnum === "MNFSR"
              ? "high"
              : "medium",
        };
    ...
    // Step 4: Clear existing wheat data (optional)
    console.log("\nStep 4: Clearing existing wheat data...");
    const deleteResult = await ProductionData.deleteMany({
      cropCode: "WHEAT",
    });
    console.log(
      `  Deleted ${deleteResult.deletedCount} existing wheat records`
    );
    ...
    try {
      ...
      const bulkResult = await ProductionData.insertMany(
      ...
      insertResult = [savedTest, ...bulkResult];
      console.log(
        `  ✅ Inserted ${insertResult.length} wheat production records (including test)`
      );
    } catch (error) {
    ...
    // Step 6: Verification
    console.log("\nStep 6: Verification...");
    const count = await ProductionData.countDocuments({ cropCode: "WHEAT" });
    console.log(`  Total wheat records in database: ${count}`);

    // Summary by year
    const pipeline = [
      { $match: { cropCode: "WHEAT" } },
      {
        $group: {
          _id: "$year",
          count: { $sum: 1 },
    ...
    console.log("\n✅ WHEAT IMPORT COMPLETED SUCCESSFULLY!");
    ...
    console.error(`❌ Error importing wheat data: ${error.message}`);
    throw error;
  }
};
